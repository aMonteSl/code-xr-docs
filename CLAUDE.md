...
- **The author portrait master lives in `assets-src/profile/adrian-montes-linares.jpg`, also outside `public/`.** `src/lib/assets.js` only ever serves the three square variants (`-small/-medium/-large`, 256/512/1024) in `public/profile/`; `npm run optimize:profile` regenerates them with sharp (manual, outside `prebuild`, like `optimize:hero`). Do not put the master back under `public/`: it is 1.35 MB that nothing fetched.
...
**`Navbar` takes `sections`, `sectionIds`, `homeHref` and `collapse` as props.** `collapse` (`'lg'` | `'xl'`, default `lg`) is the breakpoint at which that page's own link list stops fitting one 64px bar line: the home passes `xl` because eleven entries need 1280px, the subpages keep `lg`. It is not cosmetic — `scroll-padding-top` in `main.css`, `OFFSET` in `useActiveSection.js` and the subpages' `pt-16` are all calibrated against a single-line bar, and that prop is what guarantees one. The home passes `nav.sections` + `#hero`; each detail page passes **its own** list and gets `homeHref: '/'`; the `/analysis/` index passes nothing. This is not cosmetic: the section links are in-page `#` anchors, so another page's list would point at ids that do not exist, and `useActiveSection` has a bottom-of-page branch (`setActiveId(ids.at(-1))`) that a short subpage reaches immediately — with the home's list it would permanently highlight the *last* home section. An empty list makes `[].at(-1) ?? null` null, which is right for the index.
